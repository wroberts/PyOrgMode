...
class OrgDate:
    ...
    def set_value(self, value):
        ...
        search_re = r'{start}(?P<datetime>{date}(\s+{time})?){end}'.format(
            **self.DICT_RE)
        match = re.search(search_re, value)
        if match:
            timed, weekdayed, self.value = self.parse_datetime(
...
class OrgPlugin:
    ...
    def __init__(self):
        ...
        self.treated = True
        # By default, the plugin system stores the indentation before
        # the treatment
    ...
    def treat(self, current, line):
        """
        This is a wrapper function for _treat.

        Asks the plugin if he can manage this kind of line. Returns
        True if it can.
        """
        self.treated = True
        if self.keepindent:
    ...
    def _treat(self, current, line):
        """
        This is the function used by the plugin for the management of the
        line.
        """
        self.treated = False
        return current
    ...
    def _close(self, current):
        """
        This is the function used by the plugin to close everything that
        have been opened.
        """
        self.treated = False
        return current
...
class OrgClock(OrgPlugin):
    ...
    def __init__(self):
        ...
        self.regexp = re.compile(r"(?:\s*)CLOCK:(?:\s*)((?:<|\[).*(?:>||\]))"
                                 r"--((?:<|\[).*(?:>||\])).*=>\s*(.*)")

    def _treat(self, current, line):
        clocked = self.regexp.findall(line)
        if clocked:
            self._append(current, self.Element(clocked[0][0], clocked[0][1],
                                               clocked[0][2]))
        else:
            self.treated = False
        return current
    ...
    class Element(OrgElement):
        ...
        def _output(self):
            """
            Output the Clock element in text format.

            (e.g CLOCK: [2010-11-20 Sun 19:42]--[2010-11-20 Sun 20:14]
            => 0:32)
            """
...
class OrgSchedule(OrgPlugin):
    ...
    def __init__(self):
        ...
        self.regexp_scheduled = re.compile(r"SCHEDULED: ((<|\[).*?(>|\])"
                                           r"(--(<|\[).*?(>|\]))?)")
        self.regexp_deadline = re.compile(r"DEADLINE: ((<|\[).*?(>|\])"
                                          r"(--(<|\[).*?(>|\]))?)")
        self.regexp_closed = re.compile(r"CLOSED: ((<|\[).*?(>|\])"
                                        r"(--(<|\[).*?(>|\]))?)")
    ...
    class Element(OrgElement):
        """
        Schedule is an element taking into account DEADLINE, SCHEDULED and
        CLOSED parameters of elements.
        """
        DEADLINE = 1
        SCHEDULED = 2
        CLOSED = 4
        ...
        def _output(self):
            """
            Outputs the Schedule element in text format.

            (e.g SCHEDULED: <2010-10-10 10:10>)
            """
            output = ""
        ...
        if isinstance(current, OrgDrawer.Element):  # We are in a drawer
            if drawer:
                if drawer.group(1) == "END":  # Ending drawer
                    ...
                    self._append(current, self.Property(drawer.group(1),
                                                        drawer.group(2)))
...
class OrgNode(OrgPlugin):
    ...
    def _treat(self, current, line):
        ...
        if heading:  # We have a heading
            ...
            # Remove tags
            current.heading = re.sub(r":([\w]+):", "", heading[0][2])
            current.priority = heading[0][1]
            current.parent = parent

            # Looking for tags
            heading_without_links = re.sub(r" \[(.+)\]", "", heading[0][2])
            current.tags = re.findall(r":([\w]+):", heading_without_links)
        else:
            self.treated = False
        return current
